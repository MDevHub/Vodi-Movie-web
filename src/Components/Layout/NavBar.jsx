  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      try {
        ...
        if (data.results) setSearchResults(data.results.slice(0, 8));
      } catch (err) {
    ...
    <>
      <nav className="bg-black text-white p-4 flex items-center justify-between px-[6vw] fixed top-0 w-full z-50">
        <div className="flex items-center space-x-4">
          <button aria-label="Menu" onClick={toggleMenu} className="md:hidden">
            <FaBars size={17} className="text-gray-100" />
          </button>
          <Link to="/home" className="text-3xl font-bold font-poppins">CineMa</Link>
        </div>
        ...
        >
          <div className="flex flex-col space-y-6">
            ...
            <Link to="/blog" onClick={() => setMenuOpen(false)} className="hover:text-gray-400">Blog</Link>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-6">
          ...
          {/* Profile Dropdown (Now BEFORE Search) */}
          <div className="relative" ref={dropdownRef}>
            <button onClick={toggleDropdown} className="flex items-center space-x-1 hover:text-gray-400 focus:outline-none">
          ...
          <div className="relative" ref={searchRef}>
            ...
            <div className="hidden md:block relative">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2 rounded-full border border-gray-600 bg-gray-800 text-white focus:outline-none focus:ring-1 focus:ring-blue-500 transition-all duration-300"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              />
              {searchResults.length > 0 && (
                <div className="absolute top-full mt-2 w-full max-h-60 overflow-y-auto bg-black text-white rounded shadow-lg scrollbar-none">
      ...
      {mobileSearchOpen && (
        <div className="md:hidden bg-black px-4 pb-4 pt-2 fixed top-[68px] w-full z-40">
          ...
          />
          {searchResults.length > 0 && (
            <div className="mt-2 max-h-60 overflow-y-auto bg-black text-white rounded shadow-lg scrollbar-none">
              <style>{`.scrollbar-none::-webkit-scrollbar { display: none; }`}</style>
              {searchResults.map((result) => (
                <div
                  key={result.id}
