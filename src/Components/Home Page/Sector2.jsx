    ...
    const fetchTopMovies = async () => {
      try {
        const res = await axios.get(
          `https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&primary_release_year=2025&sort_by=popularity.desc${
            filter ? `&with_genres=${filter}` : ''
          }`
        );
        setVideos(res.data.results.slice(0, 20)); // fetch up to 20 for scroll
      } catch (error) {
