  ...
  const fetchVideos = async (type) => {
    try {
      ...
      if (type === 'day' || type === 'week') {
        url = `https://api.themoviedb.org/3/trending/tv/${type}?api_key=${API_KEY}`;
      } else if (type === 'month') {
        const oneMonthAgo = new Date();
        oneMonthAgo.setMonth(oneMonthAgo.getMonth() - 1);
        const date = oneMonthAgo.toISOString().split('T')[0];
        url = `https://api.themoviedb.org/3/discover/tv?api_key=${API_KEY}&sort_by=popularity.desc&first_air_date.gte=${date}`;
      } else if (type === '3month') {
        const threeMonthsAgo = new Date();
        threeMonthsAgo.setMonth(threeMonthsAgo.getMonth() - 3);
        const date = threeMonthsAgo.toISOString().split('T')[0];
        url = `https://api.themoviedb.org/3/discover/tv?api_key=${API_KEY}&sort_by=popularity.desc&first_air_date.gte=${date}`;
      }
          ...
          {[
            ...
            <button
              ...
              className={`hover:text-sky-500 whitespace-nowrap ${
                activePeriod === value ? 'text-sky-500' : ''
              }`}
            >
              {label}
            </button>
      ...
      {videos.length >= 7 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
          <div className="relative rounded-lg overflow-hidden shadow h-full md:h-auto">
            <div className="h-full md:h-full w-full cursor-pointer">
              <Link to={`/watch/${first.id}`}>
                <img
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 content-start">
            {[...row1, ...row2].map((vid) => (
              <div key={vid.id} className="bg-white/5 p-2 rounded-lg shadow cursor-pointer">
